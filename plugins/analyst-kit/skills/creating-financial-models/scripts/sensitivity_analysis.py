...
class SensitivityAnalyzer:
    ...
    def scenario_analysis(
        ...
        Args:
            ...
            base_values: Base value per variable. When provided, every variable
                is reset to its base before each scenario and after the run.
                Without it, scenarios run cumulatively (a variable one scenario
                sets persists into the next) and the model is left holding the
                last scenario's values — the caller must reset it.
        ...
        def reset_to_base():
            if base_values:
                for var_name, value in base_values.items():
        ...
        for scenario_name, variables in scenarios.items():
            # Start each scenario from the base case, not the previous scenario
            reset_to_base()
        ...
        # Leave the model at the base case rather than the last scenario
        reset_to_base()
    ...
    def breakeven_analysis(
        ...
        # Detect whether output rises or falls with the variable (e.g. DCF
        # value falls as WACC rises) so the search narrows the correct half
        variable_update(low)
        output_at_low = output_func()
        variable_update(high)
        output_at_high = output_func()
        increasing = output_at_high >= output_at_low
        ...
        while (high - low) > tolerance:
            ...
            if abs(output - target_value) < tolerance:
                return mid
            elif (output < target_value) == increasing:
                low = mid
            else:
                high = mid
