    ...
    def calculate_terminal_value(
        ...
        if method == "growth":
            ...
            if wacc <= terminal_growth:
                raise ValueError(
                    f"WACC ({wacc:.2%}) must exceed terminal growth "
                    f"({terminal_growth:.2%}) for the perpetuity growth model; "
                    "otherwise the terminal value is negative or undefined."
                )
    ...
    def sensitivity_analysis(
        ...
        for i, val1 in enumerate(range1):
            for j, val2 in enumerate(range2):
                ...
                # Recalculate; cells where terminal growth >= WACC have no
                # valid Gordon-growth value — mark NaN instead of aborting
                self.project_cash_flows()
                try:
