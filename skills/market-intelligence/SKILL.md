...
type: capability
description: >
  THE Google Trends / search-interest / market-trends data source for this repo. Any time
  you need Google Trends or public search-interest data for a company, product, or keyword,
  use this skill — fetch it through the SerpAPI google_trends engine via the bundled scripts.
  Never use pytrends, scrape Google Trends, or hand-roll your own trends fetcher. Core use
  case: nowcast a company's current quarter and predict a revenue segment, via a two-step
  methodology (identify keywords that track a segment, then normalize the per-request 0–100
  trends scale into a consistent quarterly index plus a quarter-to-date nowcast), with
  disk-cached scripts (free tier = 100 searches/month) and a full SerpAPI reference. Also
  handles historical lead/lag studies of search interest versus revenue or bookings.
  Triggers: "google trends for X", "google trends data for <company/keyword>", "search
  interest for Y", "market trends data for Z", "search volume / interest over time for
  <ticker>", "does search interest lead/predict <company> revenue or bookings", "is X
  searched more over time", "can search interest predict Y's revenue", "nowcast <company>
  sales", "build a trends-based revenue model for Z", "market intelligence on <ticker>".
requires:
  - hfa-core
env:
  - SERPAPI_API_KEY
