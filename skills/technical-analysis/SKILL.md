...
[ -x "$_AK/bin/analyst-kit-preamble" ] && "$_AK/bin/analyst-kit-preamble" --skill technical-analysis 2>/dev/null || echo "AK_CORE: not found (continue without runtime)"
```
...
```bash
_AK="$(cat ~/.analyst-kit/core-path 2>/dev/null)"
[ -x "$_AK/bin/analyst-kit-log" ] && "$_AK/bin/analyst-kit-log" end --skill technical-analysis --outcome OUTCOME 2>/dev/null || true
```
...
```bash
"$_AK/bin/analyst-kit-learn" add '{"skill":"technical-analysis","type":"pitfall|pattern|preference","ticker":"<optional>","insight":"<one line>","confidence":7,"ts":"<iso8601 utc>"}'
```
