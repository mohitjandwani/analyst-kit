...
[ -x "$_HFA/bin/hfa-preamble" ] && "$_HFA/bin/hfa-preamble" --skill single-stock-deep-dive 2>/dev/null || echo "HFA_CORE: not found (continue without runtime)"
```
...
```bash
_HFA="$(cat ~/.hfa/core-path 2>/dev/null)"
[ -x "$_HFA/bin/hfa-log" ] && "$_HFA/bin/hfa-log" end --skill single-stock-deep-dive --outcome OUTCOME 2>/dev/null || true
```
...
```bash
"$_HFA/bin/hfa-learn" add '{"skill":"single-stock-deep-dive","type":"pitfall|pattern|preference","ticker":"<optional>","insight":"<one line>","confidence":7,"ts":"<iso8601 utc>"}'
```
<!-- hfa:epilogue:end -->
