...
const HFA = new Function(`${HELPERS_JS}\nreturn HFA;`)() as {
  fmt: (v: number | null, o?: Record<string, unknown>) => string;
};

describe('HFA.fmt (shipped browser formatter)', () => {
  it('formats scaled money with currency + unit', () => {
    expect(HFA.fmt(416.2, { currency: '$', unit: 'B' })).toBe('$416.2B');
  });

  it('uses accounting parentheses for negatives', () => {
    expect(HFA.fmt(-2.5, { currency: '$', unit: 'M' })).toBe('($2.5M)');
  });

  it('formats percent', () => {
    expect(HFA.fmt(26.915, { percent: true })).toBe('26.9%');
  });

  it('never emits a raw large number — always scaled+unit', () => {
    // 12.312135 (already scaled to M by the data layer) → "$12.3M", not 12312135.
    expect(HFA.fmt(12.312135, { currency: '$', unit: 'M' })).toBe('$12.3M');
  });

  it('special-cases zero and null', () => {
    expect(HFA.fmt(0, { currency: '$', unit: 'B' })).toBe('$0');
    expect(HFA.fmt(null)).toBe('—');
  });

  it('honors fixed decimals (EPS/per-share = 2 dp, even whole values)', () => {
    expect(HFA.fmt(1, { currency: '$', decimals: 2 })).toBe('$1.00');
    expect(HFA.fmt(-0.45, { currency: '$', decimals: 2 })).toBe('($0.45)');
  });
});
